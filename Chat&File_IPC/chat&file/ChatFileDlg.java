...
public class ChatFileDlg extends JFrame implements BaseLayer {
	...
	public static void main(String[] args) {
		...
		m_LayerMgr.GetLayer(1).SetUpperLayer(m_LayerMgr.GetLayer(2));
		m_LayerMgr.GetLayer(2).SetUpperLayer(m_LayerMgr.GetLayer(4));
		m_LayerMgr.GetLayer(4).SetUnderLayer(m_LayerMgr.GetLayer(2));
		m_LayerMgr.GetLayer(2).SetUnderLayer(m_LayerMgr.GetLayer(1));
	...
	public ChatFileDlg(String pName) {
		...
		filePanel.setBorder(new TitledBorder(UIManager.getBorder("TitledBorder.border"), "파일 전송", TitledBorder.LEADING,
		...
		filePanel.add(this.progressBar.file_progress);
		// file panel end
		...
		Setting_file_Button.addActionListener(new setAddressListener());
		filePanel.add(this.Setting_file_Button);

		this.File_send_Button = new JButton("전송");
		this.File_send_Button.setBounds(275, 50, 75, 25);
		this.File_send_Button.addActionListener(new setAddressListener());
		...
		public void actionPerformed(ActionEvent e) {
			...
			if (e.getSource() == Setting_Button) { // setting 버튼 누를 시
				...
				if (Setting_Button.getText() == "Reset") { // reset 눌려졌을 경우,
					...
					((EthernetLayer) m_LayerMgr.GetLayer("Ethernet")).SetEnetSrcAddress(srcAddress); // 이부분을 통해 선택한 주소를
																										// 프로그램 상 소스주소로
																										// 사용가능
					((EthernetLayer) m_LayerMgr.GetLayer("Ethernet")).SetEnetDstAddress(dstAddress); // 이부분을 통해 선택한 주소를
																										// 프로그램 상 목적지주소로
																										// 사용가능
			...
			if (e.getSource() == Setting_file_Button) { // file 선택버튼 누른 경우
				chooser = new JFileChooser();
				chooser.setDialogTitle("파일 선택");
				//FileNameExtensionFilter filter = new FileNameExtensionFilter("보내고자하는 file 선택");
			...
			if (e.getSource() == File_send_Button) { // file 전송 버튼
				Setting_file_Button.setEnabled(false);
				((FileAppLayer) m_LayerMgr.GetLayer("FileApp")).setAndStartSendFile();
				//전송이 완료될 때까지 대기
				while (progressBar.file_progress.getValue() != progressBar.file_progress.getMaximum());
				Setting_file_Button.setEnabled(true);
				File_send_Button.setEnabled(false);
			}
		}
	}
	...
	class ProgressBar {
		...
		public ProgressBar() {
			this.file_progress = new JProgressBar(min, max);
			this.file_progress.setStringPainted(true);
		}
		public void setMinimum(int min) {
			this.min = min;
			this.file_progress.setMinimum(min);
		}
		public void setMaximum(int max) {
			this.max = max;
			this.file_progress.setMaximum(max);
		}
		public void setValue(int value) {
			this.value = value;
			this.file_progress.setValue(value);
		}
	}
